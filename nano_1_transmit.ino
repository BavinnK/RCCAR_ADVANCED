...
enum btn_map{
  ...
  btn_forward_right=PD5,
   btn_right=PC3,
   btn_backward_right=PC2,
   btn_backward=PC1,
   btn_backward_left=PC0,
   btn_left=PD4,
   btn_forward_left=PD6,
   btn_strfe_right=PC5,
   btn_stfre_left=PC4
...
void setup() {
  ...
  DDRD&=~((1<<btn_forward)|(1<<btn_forward_right)|(1<<btn_forward_left)|(1<<btn_left));
  DDRC&=~((1<<btn_right)|(1<<btn_backward_right)|(1<<btn_backward)|(1<<btn_backward_left)|(1<<btn_strfe_right)|(1<<btn_stfre_left));
  PORTD|=(1<<btn_forward)|(1<<btn_forward_right)|(1<<btn_forward_left)|(1<<btn_left);
  PORTC|=(1<<btn_right)|(1<<btn_backward_right)|(1<<btn_backward)|(1<<btn_backward_left)|(1<<btn_strfe_right)|(1<<btn_stfre_left);
...
void loop() {
  ...
  else if(!(PINC&(1<<btn_right)))         packet.btn_right=1;
  else if(!(PINC&(1<<btn_backward_right)))packet.btn_backward_right=1;
  else if(!(PINC&(1<<btn_backward)))      packet.btn_backward=1;
  else if(!(PINC&(1<<btn_backward_left))) packet.btn_backward_left=1;
  else if(!(PIND&(1<<btn_left)))          packet.btn_left=1;
  else if(!(PIND&(1<<btn_forward_left)))  packet.btn_forward_left=1;
  else if(!(PINC&(1<<btn_stfre_left)))    packet.btn_stfre_left=1;
  else if(!(PINC&(1<<btn_strfe_right)))   packet.btn_strfe_right=1;
 // if(!(PIND&(1<<btn_forward))) Serial.println("forward");
